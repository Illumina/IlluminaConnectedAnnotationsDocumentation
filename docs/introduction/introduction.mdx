...
With release GRCh38.p14, we have fixed this issue and the GRCh38.p14 release will also backward compatible with previous release. Old transcript and gene model files and all of 
...
:::note
For GRCh37 Ensembl release, despite the latest version is 110 (and they also have newer release version 111 and 112), the annotation data is effectively the same as version 87 
which was release back in 2017.
...
For gene symbols that we supported for the transcript model above, the latest data is from [HGNC](https://ftp.ebi.ac.uk/pub/databases/genenames/hgnc/json/hgnc_complete_set.json) 
website as of 2024-06-03. 

More supporting versions can be listed and downloaded using our [Downloader Manager](../utilities/data-manager.mdx).
