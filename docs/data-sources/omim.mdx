...
Illumina Connected Annotations retrieves the OMIM annotations from the [OMIM API](https://www.omim.org/api) JSON responses. The "entry" handler is used to fetch all the annotations associated with a given OMIM gene. A sample JSON response from the API is provided there.
...
Content from the OMIM API JSON response is reorganized as shown in the Illumina Connected Annotations [JSON Output](#json-output)
