...
This enhanced matching is available in version **3.27.0** and later only.
...
- **Point variants**: Insertions and translocations - treated as events occurring at specific genomic positions
- **Interval variants**: Deletions, duplications, and copy number variants - treated as events affecting genomic regions

### Matching Strategies
The system uses two different matching strategies depending on the variant type:

1. **Windowed Position Matching**: Used for point variants (insertions, translocations)
2. **Reciprocal Overlap Matching**: Used for interval variants (deletions, duplications, CNVs)
...
|-------------------|----------|-------------------|----------------------------|-------------------|
| **deletion** | interval | Reciprocal overlap (≥50%) | deletion<br/>copy_number_loss | insertion, duplication, translocation |
| **insertion** | point | Windowed position (±500bp) | insertion<br/>novel_sequence_insertion<br/>mobile_element_insertion | deletion, duplication, translocation |
| **duplication** | interval | Reciprocal overlap (≥50%) | duplication<br/>tandem_duplication<br/>copy_number_gain | deletion, insertion, translocation |
| **tandem_duplication** | interval | Reciprocal overlap (≥50%) | duplication<br/>tandem_duplication<br/>copy_number_gain | deletion, insertion, translocation |
| **translocation** | point | Windowed position (±500bp) | translocation only | All other types |
| **copy_number_gain** | interval | Reciprocal overlap (≥50%) | duplication<br/>tandem_duplication<br/>copy_number_gain | deletion, insertion, translocation |
| **copy_number_loss** | interval | Reciprocal overlap (≥50%) | deletion<br/>copy_number_loss | insertion, duplication, translocation |
| **copy_number_variation** | interval | Reciprocal overlap (≥50%) | copy_number_variation<br/>deletion<br/>duplication<br/>copy_number_loss<br/>copy_number_gain | insertion, translocation |
| **mobile_element_insertion** | point | Windowed position (±500bp) | insertion<br/>novel_sequence_insertion<br/>mobile_element_insertion | deletion, duplication, translocation |
| **mobile_element_deletion** | interval | Reciprocal overlap (≥50%) | deletion<br/>copy_number_loss | insertion, duplication, translocation |
...
- **Copy number variation** matches all interval variants but not point variants
- **Translocation** is exclusive and only matches other translocations
- **point matching** uses ±500bp windows; **interval matching** requires ≥50% reciprocal overlap
:::
...
- **Rationale**: interval variants should have substantial positional overlap to be considered equivalent
...
- **Missing Overlap Data**: interval variants require overlap calculations; point variants use windowed matching as fallback
...
### Scenario 1: Deletion Query (interval Matching)
...
- **interval variants** (deletions) require ≥50% reciprocal overlap with compatible types
- **point variants** (insertions) need to be within ±500bp window with compatible types
